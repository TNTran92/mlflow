"""
This is an experimental sktime benchmkaring implemenmtation

Sktime (native) format
    This is the main flavor that can be accessed with Sktime APIs.
...
_MODEL_BINARY_FILE_NAME = "model.sktime"
...
def save_model(
    ...
    :param sktime_model: sktime model/estimator (an instance of Prophet() forecaster that has been fit
    ...
    pyfunc.add_to_model(
        mlflow_model,
        loader_module="mlflow.sktime_beta",
    ...
    mlflow_model.add_flavor(
        FLAVOR_NAME,
        prophet_version=sktime.__version__,
        code=code_dir_subpath,
        **flavor_conf,
    )
...
def log_model(
    ...
    :param sktime_model: Prophet model to be saved.
...
def load_model(model_uri, dst_path=None):
    ...
    sktime_model_path = os.path.join(
        local_model_path, flavor_conf.get(_MODEL_BINARY_KEY, _MODEL_BINARY_FILE_NAME)
    )

    return _load_model(sktime_model_path)


class _sktimeWrapper:
